...
public class TaxWhiz {
    ...
    /**
     * @param newRate -- new tax rate Changes tax rate by incrementing
     */
    ...
    /**
     * @param args Creates two TaxWhiz objects to store tax rates, creates a number
     *             format for currency object, declares the cost of two items and
     *             passes it all to a receipt printing function. After this, the tax
     *             rates are changed and everything is passed again to reprint a new
     *             receipt.
     */
    public static void main(String args[]) {
        ...
        printReceipt(taxRateOne, taxRateTwo, currency, appleCost, jacketCost);
     ...
     */
    public static void printReceipt(TaxWhiz taxRateOne, TaxWhiz taxRateTwo, NumberFormat currency, double appleCost,
            double jacketCost) {

        // calculates all of the amounts, sums them then formats with a currency format
        String total = currency
                .format((appleCost + taxRateOne.calcTax(appleCost)) + (jacketCost + taxRateTwo.calcTax(jacketCost)));

        // joins all of the variables together to calculate and print cost, tax and
        // total
        String receipt = "\n--------------------------------\n" + "Clerk:\tJoel\n--------------\n"
                + "\nItem: Apples 12oz\tCost: " + currency.format(appleCost) + "\t\tTax: "
                + currency.format(taxRateOne.calcTax(appleCost)) + "\tItem Total: "
                + currency.format(appleCost + taxRateOne.calcTax(appleCost)) + "\nItem: Jacket\t\tCost: "
                + currency.format(jacketCost) + "\tTax: " + currency.format(taxRateTwo.calcTax(jacketCost))
                + "\tItem Total: " + currency.format(jacketCost + taxRateTwo.calcTax(jacketCost)) + "\n\nSubtotal:\t"
                + currency.format(appleCost + jacketCost) + "\nTotal Tax:\t"
                + currency.format(taxRateOne.calcTax(appleCost) + taxRateTwo.calcTax(jacketCost)) + "\nTotal:\t\t"
